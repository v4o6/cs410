...
sub ParseEvent {
	...
	my $threadID = &GetThreadID($argumentList);
	
	# At this point, should update nodes based on new information.
	$nodes{threadID} = {
		timestamp => $timestamp,
		threadNum => $threadNum,
		enterExit => $enterExit,
		functionName => $functionName,
		stackOrReturn => $stackOrReturn
	};
}

sub WriteDotFile {
	# Open a dot file for writing graph code.
	open (GRAPHFILE, ">", "graph.dot");
	# Write dot file ..
	print GRAPHFILE "digraph G {\n";
	foreach my $key ( keys %nodes )
	{
		#print "key: $key, value: $hash{$key}{'timestamp'}\n";
		# Scenarios for different functions. Assuming that all pthread calls will
		# pass, besides the ones which wait.
